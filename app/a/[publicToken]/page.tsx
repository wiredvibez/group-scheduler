...
import { Appointment, ContactFieldKey, ResponseContact, TimeOption, VoteResponse } from "@/lib/types";
...
export default function PublicVotePage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const appointmentSnap = await getDocs(
        query(collection(db, "appointments"), where("publicToken", "==", token), limit(1)),
      );
      ...
      const optionsSnap = await getDocs(collection(db, "appointments", item.id, "timeOptions"));
      setOptions(
  ...
  const maxAllowed = useMemo(() => {
    ...
    if (appointment.voteMode === "single") {
      return 1;
    }
    ...
    return (
      <main className="page-shell">
        <section className="panel max-w-2xl">
          <p className="msg-error" role="alert">{error || "שגיאה בטעינה"}</p>
        </section>
      </main>
    );
  }
  ...
  const enabledContactFields = (Object.keys(CONTACT_LABELS) as ContactFieldKey[]).filter(
    (key) => appointment.contactFieldsConfig?.[key]?.enabled,
  );

  const validateContactFields = (): string | null => {
    for (const key of enabledContactFields) {
      if (appointment.contactFieldsConfig[key].required && !String(contact[key] || "").trim()) {
  ...
  const upsert = async (targetResponseId?: string, mode: "submit" | "update" = "submit") => {
    const responseId = targetResponseId || crypto.randomUUID();
    await setDoc(doc(db, "appointments", appointmentId, "responses", responseId), {
      deviceId,
      selectedOptionIds: selected,
      contact,
      note: note.trim(),
      status: mode === "submit" ? "submitted" : "updated",
      createdAt: existingResponseId || targetResponseId ? undefined : serverTimestamp(),
      updatedAt: serverTimestamp(),
    }, { merge: true });

    await setDoc(doc(db, "appointments", appointmentId, "activity", crypto.randomUUID()), {
      type: mode,
      responseId,
      deviceId,
    ...
    <main className="page-shell">
      <section className="panel w-full max-w-4xl">
        ...
        {appointment.description ? (
          <p className="mt-2 text-[var(--text-muted)]">{appointment.description}</p>
        ) : null}
        ...
        <section className="mt-6">
          ...
          <div className="mt-3 grid gap-2">
            {options.map((option) => (
              ...
              >
                <span className="text-lg font-bold">{formatDateTime(option.startAt)}</span>
              </button>
            ))}
          </div>
        </section>
            ...
            <label className="field" key={fieldKey}>
              ...
              {fieldKey === "gender" ? (
                <select
                  ...
                  onChange={(e) => setContact((prev) => ({ ...prev, [fieldKey]: e.target.value }))}
                >
        ...
        {duplicateResponseId ? (
          <div className="mt-5 tile border-[var(--danger)] bg-[var(--danger-bg)]">
            <p className="font-bold">נמצאה הצבעה קיימת עם פרטי הקשר האלו.</p>
            <p className="mt-1 text-[var(--text)]">האם לעדכן את ההצבעה הקיימת?</p>
        ...
        >
          {saving ? "שומר..." : existingResponseId ? "עדכן הצבעה" : "שלח הצבעה"}
        </button>
      </section>
    </main>
  );
